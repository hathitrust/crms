...
use lib "$ENV{SDRROOT}/crms/lib/CRMS";
...
package CRMS::DB::CRMS {
  ...
  # The database name (db_name) from config.
  # CRMS_DB_NAME or CRMS_HT_DB_NAME
  sub name {
    my $self = shift;
  ...
  # The database host (db_host) from config.
  # CRMS_DB_HOST or CRMS_HT_DB_HOST
  sub host {
    my $self = shift;
  
    return $self->{config}->config->{db_host};
  }
  ...
  sub credentials {
    ...
    return {
      user => $self->{config}->credentials->{db_user},
      passwd => $self->{config}->credentials->{db_password}
    };
  }
}
