...
our @EXPORT = qw(Tables TableQuery Queries Query Translation);
...
sub Translation
{
  my $self = shift;
  my $word = shift;
  my $keio_translations = {'添付番号' => 'Attachment Number',
                           '著者名' => 'Author Name',
  ...
  my $kt = { map {my $tmp = $_; my $tmp2 = $keio_translations->{$_};
                  Encode::decode('UTF-8', $tmp, Encode::FB_CROAK) =>
                  Encode::decode('UTF-8', $tmp2, Encode::FB_CROAK)} keys %$keio_translations };
  return $kt->{$word};
}
